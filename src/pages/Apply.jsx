...
const EMPTY_AUTHOR = { prefix: "", name: "", department: "", organization: "", contact: "", email: "" };
...
const INIT = {
  ...
  authors: [],
  journal: "", publisher: "", publishingDate: "",
...
function validate(form) {
  ...
  if (form.accessType === "Open Access" && !String(form.openAccessAmount || "").trim()) e.openAccessAmount = "Please enter the open access fee paid (INR).";
...
export default function Apply() {
  ...
  const [modal, setModal]       = useState(null);  // { type: "success"|"duplicate"|"error", ackNumber?, message? }
  ...
  const handleChange = (e) => {
    ...
    if (name === "paperType") {
      setForm((p) => ({ ...p, paperType: value, quartile: "", accessType: "", openAccessAmount: "" }));
  ...
  /* ── Author management ── */
  const addAuthor = () => {
    if (form.authors.length >= MAX_AUTHORS) return;
    setForm((p) => ({ ...p, authors: [...p.authors, { ...EMPTY_AUTHOR }] }));
  };
  ...
  const updateAuthor = (idx, field, value) => {
    setForm((p) => ({
      ...p,
      authors: p.authors.map((a, i) => i === idx ? { ...a, [field]: value } : a),
    }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    /* Domain check on client before hitting server */
    ...
    try {
      ...
      if (err.error === "DUPLICATE") {
        setModal({ type: "duplicate", message: err.message });
      } else if (err.error === "INVALID_DOMAIN") {
        setErrors((p) => ({ ...p, email: err.message }));
      } else {
        setModal({ type: "error", message: err.message || "Submission failed. Please try again." });
      }
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <div className="apply-page">
      <div className="page-header">
        ...
        <p className="page-subtitle">Fields marked <span style={{ color: "var(--orange)" }}>*</span> are mandatory. DOI and preprint status are optional.</p>
      </div>
        ...
        <form onSubmit={handleSubmit} noValidate>
          ...
          <div className="form-grid">
            ...
            <Field id="email" label="Email Address" required error={errors.email}>
              <input id="email" name="email" type="email" className="form-input" placeholder="name@jssstu.edu.in" value={form.email} onChange={handleChange} />
            </Field>
          </div>
          ...
          {/* ── Co/Additional Authors — visible after authorType is selected ── */}
          {showAuthors && (
            <div className="conditional-field" style={{ marginBottom: "0.5rem" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "0.75rem" }}>
                <span style={{ fontSize: "0.78rem", fontWeight: 600, color: "var(--text-secondary)", textTransform: "uppercase", letterSpacing: "0.06em" }}>
                  Additional Authors
                  <span style={{ marginLeft: "0.4rem", fontWeight: 400, color: "var(--text-muted)", textTransform: "none" }}>
              ...
              {form.authors.map((author, idx) => (
                <div className="coauthor-card" key={idx}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "0.9rem" }}>
                    ...
                    <button type="button" className="btn-remove" onClick={() => removeAuthor(idx)}>
                      <XIcon /> Remove
                    </button>
                  </div>
                  <div className="form-grid-3">
                  ...
                  <div className="form-grid">
                    ...
                    <div className="form-group">
                      ...
                      <input type="text" className="form-input" placeholder="University / Institution" value={author.organization} onChange={(e) => updateAuthor(idx, "organization", e.target.value)} />
                    </div>
                  </div>
                  <div className="form-grid">
                    <div className="form-group">
                      <label className="form-label">Contact Number</label>
                      <input type="tel" className="form-input" placeholder="+91 XXXXX XXXXX" value={author.contact} onChange={(e) => updateAuthor(idx, "contact", e.target.value)} />
                    </div>
                    <div className="form-group">
          ...
          {/* Type of Access — only for Journal, Book Chapter, Book */}
          {showAccess && (
            <div className="conditional-field">
              ...
              {/* Open Access fee — only when Open Access is selected */}
              {form.accessType === "Open Access" && (
                <div className="conditional-field">
                  <Field
                    id="openAccessAmount"
                    label="Open Access Fee Paid (INR)"
                    required
                    error={errors.openAccessAmount}
                    hint="Enter the APC (Article Processing Charge) paid in Indian Rupees"
                  >
                    <div style={{ position: "relative" }}>
                      ...
                      <input
                        id="openAccessAmount"
                        name="openAccessAmount"
                        type="number"
                        min="0"
                        className="form-input"
                        style={{ paddingLeft: "2rem" }}
                        placeholder="e.g. 25000"
                        value={form.openAccessAmount || ""}
          ...
          <div className="form-group">
            ...
            <div className="toggle-group">
              <button
                type="button"
                className={`toggle-btn ${form.preprintAvailable === "yes" ? "active-yes" : ""}`}
                onClick={() => setForm((p) => ({ ...p, preprintAvailable: p.preprintAvailable === "yes" ? "" : "yes" }))}
              >
                Yes
              </button>
              <button
                type="button"
                className={`toggle-btn ${form.preprintAvailable === "no" ? "active-no" : ""}`}
                onClick={() => setForm((p) => ({ ...p, preprintAvailable: p.preprintAvailable === "no" ? "" : "no" }))}
              >
                No
              </button>
            </div>
          ...
          <div className="form-group">
            ...
            {(fileError || errors.paperFile) && (
              <span className="file-upload-error" role="alert">{fileError || errors.paperFile}</span>
            )}
          </div>
          ...
          <div className="submit-row">
            <button
              ...
              style={{ minWidth: "162px", padding: "0.8rem 2.2rem", opacity: submitting ? 0.7 : 1 }}
              disabled={submitting}
            >
        ...
        <GlassCard>
          <div className="contact-inner">
            <div>
              ...
              <p className="contact-desc">Facing issues with submission? Reach out to the Research Cell for support.</p>
            </div>
      ...
      {modal?.type === "success" && (
        <div className="modal-overlay" role="dialog" aria-modal="true" aria-labelledby="modal-title">
          <div className="modal-box">
            ...
            <div style={{ margin: "0.75rem 0 1.2rem", padding: "1rem 1.2rem", background: "rgba(245,110,0,0.08)", border: "1px solid rgba(245,110,0,0.2)", borderRadius: "12px", textAlign: "center" }}>
            ...
            <p className="modal-message">Your paper has been registered. A confirmation email with your acknowledgement number has been sent to your registered email address.</p>
            <button className="btn-primary" onClick={() => setModal(null)} autoFocus style={{ minWidth: "120px" }}>Done</button>
          </div>
        </div>
      )}
